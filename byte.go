...
// ByteIterator is a pull-based iterator that iterates
// over the children of a Byte.
...
// node is never nil.
func (iter *ByteIterator) Consume() (Noder, error) {
	if iter.current == nil {
...
func (iter *ByteIterator) Restart() {
	iter.current = iter.parent.FirstChild
}

// Byte is a node in a tree.
type Byte struct {
	Parent, FirstChild, NextSibling, LastChild, PrevSibling *Byte
	Data byte
}
...
func (b *Byte) Iterator() common.Iterater[Noder] {
	return &ByteIterator{
		parent: b,
		current: b.FirstChild,
	}
}
...
func (b *Byte) Copy() common.Copier {
	...
	b_copy.LinkChildren(child_copy)

	return b_copy
}

// SetParent implements the Noder interface.
func (b *Byte) SetParent(parent Noder) bool {
	if parent == nil {
		b.Parent = nil
		return true
	}
...
func (b *Byte) GetParent() Noder {
	return b.Parent
}

// LinkWithParent implements the Noder interface.
//
// Children that are not of type *Byte or nil are ignored.
func (b *Byte) LinkChildren(children []Noder) {
	if len(children) == 0 {
		return
	}

	var valid_children []*Byte
	...
	b.FirstChild, b.LastChild = valid_children[0], valid_children[len(valid_children)-1]
}
...
func (b *Byte) GetLeaves() []Noder {
	...
	stack := Stacker.NewLinkedStack[Noder](b)
...
//
// Finally, no nil nodes are returned.
func (b *Byte) GetAncestors() []Noder {
	var ancestors []Noder

	for node := b; node.Parent != nil; node = node.Parent {
...
func (b *Byte) IsLeaf() bool {
	return b.FirstChild == nil
}

// IsSingleton implements the Noder interface.
func (b *Byte) IsSingleton() bool {
	return b.FirstChild != nil && b.FirstChild == b.LastChild
}

// GetFirstChild implements the Noder interface.
func (b *Byte) GetFirstChild() Noder {
	return b.FirstChild
}
...
func (b *Byte) DeleteChild(target Noder) []Noder {
	if target == nil {
		return nil
	}
...
func (b *Byte) Size() int {
	...
	// it is not limited in size. If it is, make sure to check the error
	// returned by the Push and Pop methods.
	stack := Stacker.NewLinkedStack(b)
...
func (b *Byte) AddChild(child Noder) {
	if child == nil {
		return
	}
...
func (b *Byte) RemoveNode() []Noder {
	...
	if parent == nil {
		...
		children := parent.delete_child(b)
...
// NewByte creates a new node with the given data.
//
// Parameters:
//   - Data: The Data of the node.
//
// Returns:
//   - *Byte: A pointer to the newly created node. It is
//   never nil.
...
//
// Returns:
//   - *Byte: A pointer to the last sibling.
func (b *Byte) GetLastSibling() *Byte {
	if b.Parent != nil {
		return b.Parent.LastChild
	} else if b.NextSibling == nil {
		return b
	}
...
//
// Returns:
//   - *Byte: A pointer to the first sibling.
func (b *Byte) GetFirstSibling() *Byte {
	if b.Parent != nil {
		return b.Parent.FirstChild
	} else if b.PrevSibling == nil {
		return b
	}
...
func (b *Byte) IsRoot() bool {
	return b.Parent == nil
}

// AddChildren is a convenience function to add multiple children to the node at once.
// It is more efficient than adding them one by one. Therefore, the behaviors are the
// same as the behaviors of the Byte.AddChild function.
//
// Parameters:
//   - children: The children to add.
func (b *Byte) AddChildren(children []*Byte) {
	if len(children) == 0 {
		return
	}
...
func (b *Byte) HasChild(target *Byte) bool {
	if target == nil || b.FirstChild == nil {
		return false
	}
...
// Returns:
//   - []Noder: A slice of pointers to the children of the node.
func (b *Byte) delete_child(target *Byte) []Noder {
	ok := b.HasChild(target)
	if !ok {
		return nil
	}
	...
	if target == b.FirstChild {
		b.FirstChild = next

		if next == nil {
			b.LastChild = nil
		}
	} else if target == b.LastChild {
		b.LastChild = prev
	}
...
func (b *Byte) IsChildOf(target *Byte) bool {
	if target == nil {
		return false
	}
