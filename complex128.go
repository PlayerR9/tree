...
//
// Moreover, the return value is always of type *Complex128Node and never nil; unless the iterator
// has reached the end of the branch.
func (iter *Complex128NodeIterator) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *Complex128Node) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *Complex128Node) Cleanup() []tree.Noder {
...
func (tn *Complex128Node) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*Complex128Node)
	if !ok {
		return nil
	}
...
func (tn *Complex128Node) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *Complex128Node) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *Complex128Node) LinkChildren(children []tree.Noder) {
...
func (tn *Complex128Node) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *Complex128Node) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type Complex128Node.
func (tn *Complex128Node) Iterator() common.Iterater[tree.Noder] {
...
func (tn *Complex128Node) String() string {
	return fmt.Sprintf("Complex128Node[%v]", tn.Data)
}
