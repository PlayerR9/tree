...
//
// Moreover, the return value is always of type *Complex64Node and never nil; unless the iterator
// has reached the end of the branch.
func (iter *Complex64NodeIterator) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *Complex64Node) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *Complex64Node) Cleanup() []tree.Noder {
...
func (tn *Complex64Node) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*Complex64Node)
	if !ok {
		return nil
	}
...
func (tn *Complex64Node) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *Complex64Node) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *Complex64Node) LinkChildren(children []tree.Noder) {
...
func (tn *Complex64Node) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *Complex64Node) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type Complex64Node.
func (tn *Complex64Node) Iterator() common.Iterater[tree.Noder] {
...
func (tn *Complex64Node) String() string {
	return fmt.Sprintf("Complex64Node[%v]", tn.Data)
}
