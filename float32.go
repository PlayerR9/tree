...
// Float32NodeIterator is a pull-based iterator that iterates
// over the children of a Float32Node.
type Float32NodeIterator struct {
	parent, current *Float32Node
}
...
// node is never nil.
func (iter *Float32NodeIterator) Consume() (*Float32Node, error) {
	if iter.current == nil {
...
// Iterator implements the Float32Node interface.
//
// This function iterates over the children of the node, it is a pull-based iterator,
// and never returns nil.
func (tn *Float32Node) Iterator() common.Iterater[*Float32Node] {
	return &Float32NodeIterator{
		parent: tn,
		current: tn.FirstChild,
	}
}

// String implements the Float32Node interface.
...
// LinkWithParent implements the Float32Node interface.
//
...
func (tn *Float32Node) LinkChildren(children []*Float32Node) {
	...
	var valid_children []*Float32Node
...
// GetLeaves implements the Float32Node interface.
//
...
func (tn *Float32Node) GetLeaves() []*Float32Node {
	...
	stack := Stacker.NewLinkedStack[*Float32Node](tn)

	var leaves []*Float32Node
...
// Cleanup implements the Float32Node interface.
//
...
// GetAncestors implements the Float32Node interface.
//
...
//
// Finally, no nil nodes are returned.
func (tn *Float32Node) GetAncestors() []*Float32Node {
	var ancestors []*Float32Node
...
// IsLeaf implements the Float32Node interface.
func (tn *Float32Node) IsLeaf() bool {
	return tn.FirstChild == nil
}

// IsSingleton implements the Float32Node interface.
func (tn *Float32Node) IsSingleton() bool {
	return tn.FirstChild != nil && tn.FirstChild == tn.LastChild
}

// GetFirstChild implements the Float32Node interface.
func (tn *Float32Node) GetFirstChild() *Float32Node {
	return tn.FirstChild
}

// DeleteChild implements the Float32Node interface.
//
...
// Size implements the Float32Node interface.
//
...
func (tn *Float32Node) AddChild(child *Float32Node) {
	if child == nil {
		return
	}
...
//
// Returns:
//   - *Float32Node: A pointer to the newly created node. It is
//   never nil.
...
// same as the behaviors of the Float32Node.AddChild function.
//
// Parameters:
...
// Returns:
//   - []Float32Node: A slice of pointers to the children of the node.
func (tn *Float32Node) delete_child(target *Float32Node) []*Float32Node {
