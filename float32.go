...
//
// Moreover, the return value is always of type *Float32Node and never nil; unless the iterator
// has reached the end of the branch.
func (iter *Float32NodeIterator) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *Float32Node) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *Float32Node) Cleanup() []tree.Noder {
...
func (tn *Float32Node) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*Float32Node)
	if !ok {
		return nil
	}
...
func (tn *Float32Node) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *Float32Node) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *Float32Node) LinkChildren(children []tree.Noder) {
...
func (tn *Float32Node) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *Float32Node) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type Float32Node.
func (tn *Float32Node) Iterator() common.Iterater[tree.Noder] {
...
func (tn *Float32Node) String() string {
	return fmt.Sprintf("Float32Node[%v]", tn.Data)
}
