...
//
// Moreover, the return value is always of type *TreeNode[T] and never nil; unless the iterator
// has reached the end of the branch.
func (iter *TreeNodeIterator[T]) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *TreeNode[T]) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *TreeNode[T]) Cleanup() []tree.Noder {
...
func (tn *TreeNode[T]) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}
...
func (tn *TreeNode[T]) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *TreeNode[T]) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *TreeNode[T]) LinkChildren(children []tree.Noder) {
...
func (tn *TreeNode[T]) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *TreeNode[T]) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type TreeNode[T].
func (tn *TreeNode[T]) Iterator() common.Iterater[tree.Noder] {
...
func (tn *TreeNode[T]) String() string {
	return fmt.Sprintf("TreeNode[T][%v]", tn.Data)
}
