...
//
// Moreover, the return value is always of type *IntNode and never nil; unless the iterator
// has reached the end of the branch.
func (iter *IntNodeIterator) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *IntNode) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *IntNode) Cleanup() []tree.Noder {
...
func (tn *IntNode) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*IntNode)
	if !ok {
		return nil
	}
...
func (tn *IntNode) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *IntNode) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *IntNode) LinkChildren(children []tree.Noder) {
...
func (tn *IntNode) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *IntNode) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type IntNode.
func (tn *IntNode) Iterator() common.Iterater[tree.Noder] {
...
func (tn *IntNode) String() string {
	return fmt.Sprintf("IntNode[%v]", tn.Data)
}
