...
//
// Moreover, the return value is always of type *Int64Node and never nil; unless the iterator
// has reached the end of the branch.
func (iter *Int64NodeIterator) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *Int64Node) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *Int64Node) Cleanup() []tree.Noder {
...
func (tn *Int64Node) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*Int64Node)
	if !ok {
		return nil
	}
...
func (tn *Int64Node) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *Int64Node) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *Int64Node) LinkChildren(children []tree.Noder) {
...
func (tn *Int64Node) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *Int64Node) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type Int64Node.
func (tn *Int64Node) Iterator() common.Iterater[tree.Noder] {
...
func (tn *Int64Node) String() string {
	return fmt.Sprintf("Int64Node[%v]", tn.Data)
}
