...
//
// Moreover, the return value is always of type *RuneNode and never nil; unless the iterator
// has reached the end of the branch.
func (iter *RuneNodeIterator) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *RuneNode) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *RuneNode) Cleanup() []tree.Noder {
...
func (tn *RuneNode) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*RuneNode)
	if !ok {
		return nil
	}
...
func (tn *RuneNode) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *RuneNode) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *RuneNode) LinkChildren(children []tree.Noder) {
...
func (tn *RuneNode) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *RuneNode) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type RuneNode.
func (tn *RuneNode) Iterator() common.Iterater[tree.Noder] {
...
func (tn *RuneNode) String() string {
	return fmt.Sprintf("RuneNode[%v]", tn.Data)
}
