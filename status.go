...
//
// Moreover, the return value is always of type *StatusNode[S, T] and never nil; unless the iterator
// has reached the end of the branch.
func (iter *StatusNodeIterator[S, T]) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *StatusNode[S, T]) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *StatusNode[S, T]) Cleanup() []tree.Noder {
...
func (tn *StatusNode[S, T]) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*StatusNode[S, T])
	if !ok {
		return nil
	}
...
func (tn *StatusNode[S, T]) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *StatusNode[S, T]) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *StatusNode[S, T]) LinkChildren(children []tree.Noder) {
...
//
// Returns:
//   - []StatusNode[S, T]: A slice of pointers to the children of the node.
func (tn *StatusNode[S, T]) delete_child(target *StatusNode[S, T]) []*StatusNode[S, T] {
...
func (tn *StatusNode[S, T]) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent

	var sub_roots []*StatusNode[S, T]
...
// values is guaranteed to be a non-nil node of type StatusNode[S, T].
func (tn *StatusNode[S, T]) Iterator() common.Iterater[tree.Noder] {
...
func (tn *StatusNode[S, T]) String() string {
	return fmt.Sprintf("StatusNode[S, T][%v]", tn.Data)
}
...
// Returns:
//   - []*StatusNode[S, T]: A slice of pointers to the children of the node.
func (tn *StatusNode[S, T]) GetChildren() []*StatusNode[S, T] {
	var children []*StatusNode[S, T]
