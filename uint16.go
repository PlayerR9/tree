...
// Uint16NodeIterator is a pull-based iterator that iterates
// over the children of a Uint16Node.
type Uint16NodeIterator struct {
	parent, current *Uint16Node
}
...
//
// Moreover, the return value is always of type *Uint16Node and never nil; unless the iterator
// has reached the end of the branch.
func (iter *Uint16NodeIterator) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *Uint16Node) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *Uint16Node) Cleanup() []tree.Noder {
...
func (tn *Uint16Node) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*Uint16Node)
	if !ok {
		return nil
	}
...
func (tn *Uint16Node) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *Uint16Node) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *Uint16Node) LinkChildren(children []tree.Noder) {
...
func (tn *Uint16Node) delete_child(target *Uint16Node) []*Uint16Node {
...
func (tn *Uint16Node) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent

	var sub_roots []*Uint16Node
...
func (tn *Uint16Node) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type Uint16Node.
func (tn *Uint16Node) Iterator() common.Iterater[tree.Noder] {
	return &Uint16NodeIterator{
		parent: tn,
...
func (tn *Uint16Node) String() string {
	return fmt.Sprintf("Uint16Node[%v]", tn.Data)
}
...
// same as the behaviors of the Uint16Node.AddChild function.
//
// Parameters:
