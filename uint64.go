...
//
// Moreover, the return value is always of type *Uint64Node and never nil; unless the iterator
// has reached the end of the branch.
func (iter *Uint64NodeIterator) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *Uint64Node) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *Uint64Node) Cleanup() []tree.Noder {
...
func (tn *Uint64Node) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*Uint64Node)
	if !ok {
		return nil
	}
...
func (tn *Uint64Node) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *Uint64Node) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *Uint64Node) LinkChildren(children []tree.Noder) {
...
func (tn *Uint64Node) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *Uint64Node) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type Uint64Node.
func (tn *Uint64Node) Iterator() common.Iterater[tree.Noder] {
...
func (tn *Uint64Node) String() string {
	return fmt.Sprintf("Uint64Node[%v]", tn.Data)
}
