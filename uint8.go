...
// Uint8NodeIterator is a pull-based iterator that iterates
// over the children of a Uint8Node.
type Uint8NodeIterator struct {
	parent, current *Uint8Node
}
...
// node is never nil.
func (iter *Uint8NodeIterator) Consume() (*Uint8Node, error) {
	if iter.current == nil {
...
// Iterator implements the Uint8Node interface.
//
// This function iterates over the children of the node, it is a pull-based iterator,
// and never returns nil.
func (tn *Uint8Node) Iterator() common.Iterater[*Uint8Node] {
	return &Uint8NodeIterator{
		parent: tn,
		current: tn.FirstChild,
	}
}

// String implements the Uint8Node interface.
...
// LinkWithParent implements the Uint8Node interface.
//
...
// GetLeaves implements the Uint8Node interface.
//
...
func (tn *Uint8Node) GetLeaves() []*Uint8Node {
	...
	stack := Stacker.NewLinkedStack[*Uint8Node](tn)

	var leaves []*Uint8Node
...
// Cleanup implements the Uint8Node interface.
//
...
// GetAncestors implements the Uint8Node interface.
//
...
//
// Finally, no nil nodes are returned.
func (tn *Uint8Node) GetAncestors() []*Uint8Node {
	var ancestors []*Uint8Node
...
// IsLeaf implements the Uint8Node interface.
func (tn *Uint8Node) IsLeaf() bool {
	return tn.FirstChild == nil
}

// IsSingleton implements the Uint8Node interface.
func (tn *Uint8Node) IsSingleton() bool {
	return tn.FirstChild != nil && tn.FirstChild == tn.LastChild
}

// GetFirstChild implements the Uint8Node interface.
func (tn *Uint8Node) GetFirstChild() *Uint8Node {
	return tn.FirstChild
}

// DeleteChild implements the Uint8Node interface.
//
...
// Size implements the Uint8Node interface.
//
...
func (tn *Uint8Node) AddChild(child *Uint8Node) {
	if child == nil {
		return
	}
