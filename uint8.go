...
//
// Moreover, the return value is always of type *Uint8Node and never nil; unless the iterator
// has reached the end of the branch.
func (iter *Uint8NodeIterator) Consume() (tree.Noder, error) {
	if iter.current == nil {
...
func (tn *Uint8Node) GetParent() tree.Noder {
	return tn.Parent
}
...
func (tn *Uint8Node) Cleanup() []tree.Noder {
...
func (tn *Uint8Node) DeleteChild(target tree.Noder) []tree.Noder {
	if target == nil {
		return nil
	}

	tmp, ok := target.(*Uint8Node)
	if !ok {
		return nil
	}
...
func (tn *Uint8Node) GetFirstChild() tree.Noder {
	return tn.FirstChild
}

// AddChild implements the tree.Noder interface.
func (tn *Uint8Node) AddChild(target tree.Noder) {
	if target == nil {
		return
	}
...
func (tn *Uint8Node) LinkChildren(children []tree.Noder) {
...
func (tn *Uint8Node) RemoveNode() []tree.Noder {
	prev := tn.PrevSibling
	next := tn.NextSibling
	parent := tn.Parent
...
func (tn *Uint8Node) Copy() tree.Noder {
...
// values is guaranteed to be a non-nil node of type Uint8Node.
func (tn *Uint8Node) Iterator() common.Iterater[tree.Noder] {
...
func (tn *Uint8Node) String() string {
	return fmt.Sprintf("Uint8Node[%v]", tn.Data)
}
